...
def descriptor_from_policy(pol: ProviderPolicy) -> ProviderDescriptor:
    ...
    return ProviderDescriptor(
        ...
        supported_models=tuple(pol.model_router_prefixes) or (pol.family_id,),
        model_classes=("local_small", "local_medium") if local else ("cloud_general",),
        streaming_supported=bool(pol.streaming),
        structured_output_supported=False,  # not certified in M21.2
        tool_use_supported=bool(pol.tool_calling),
        max_context_tokens=8192 if local else 128000,
