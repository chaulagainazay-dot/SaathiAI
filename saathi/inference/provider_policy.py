...
PROVIDER_POLICIES: tuple[ProviderPolicy, ...] = (
    ...
    ProviderPolicy(
        family_id="kimi",
        provider_class=ProviderClass.CLOUD,
        kill_switch_env="SAATHI_PROVIDER_KILL_KIMI",
        production_supported=False,
        default_policy_enabled=False,
        streaming=True,
        tool_calling=True,
        local=False,
        model_router_prefixes=("kimi/",),
        llm_caller_key="kimi",
        cost=CostMetadata(
            cost_per_1k_input_usd=0.003,
            cost_per_1k_output_usd=0.015,
            relative_cost_per_1k=0.015,
            known=True,
            notes="K3 conservative cache-miss rate; model-specific adapter pricing is authoritative",
        ),
        notes="Remote benchmark candidate; K3 requires explicit expensive-model approval",
    ),
    ProviderPolicy(
        family_id="nvidia_kimi",
        provider_class=ProviderClass.CLOUD,
        kill_switch_env="SAATHI_PROVIDER_KILL_NVIDIA_KIMI",
        production_supported=False,
        default_policy_enabled=False,
        streaming=True,
        tool_calling=True,
        local=False,
        model_router_prefixes=("nvidia_kimi/",),
        llm_caller_key="nvidia_kimi",
        cost=CostMetadata(
            cost_per_1k_input_usd=0.003,
            cost_per_1k_output_usd=0.015,
            relative_cost_per_1k=0.015,
            known=True,
            notes="NVIDIA NIM Kimi K3; explicit opt-in only",
        ),
        notes="NVIDIA NIM hosted Kimi K3; cloud fallback remains disabled by default",
    ),
    ProviderPolicy(
