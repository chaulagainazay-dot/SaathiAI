...
def main(argv=None) -> int:
    ...
    if cmd == "import-cli-anything":
        ...
        # M17.18/M17.19: same authoritative validator; strict batch (no partial write)
        reg = registry.import_records(res.get("records") or [], strict=True)
        out = {"imported": res["imported"], "rejected": len(res["rejected"]),
               "trust": res["trust"], "registered": reg.get("added", 0),
               "total": reg.get("total"), "store": str(registry.STORE)}
        if reg.get("error"):
            out["error"] = reg["error"]
