...
def test_run_shell_blocked_via_execute_tool():
    out = execute_tool("run_shell", {"command": "ls -la"})
...
def test_manage_tasks_list_canonical():
    out = execute_tool("manage_tasks", {"action": "list"})
    if "error" not in out:
