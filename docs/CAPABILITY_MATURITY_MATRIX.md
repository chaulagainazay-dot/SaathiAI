...
| LocalModelHarness (FM-I5–I6.2 + MG-FIX) | deterministic-tested (mock); combined macOS memory gate **implemented** (`fm_i6_2_mg_fix.combined_macos.v1`); live **blocked** (host headroom + separate live cert); **not role-qualified**; production not authorized | `local_model_memory_gate.py`; `docs/adr/ADR-MACOS-LOCAL-MODEL-MEMORY-GATE.md`; `docs/evidence/fm_i6_2_memory_gate_fix/`; pure free diagnostic only; `PRODUCTION_CERTIFIED=False` |
