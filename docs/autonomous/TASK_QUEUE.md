# Autonomous Task Queue

## Active

- M68 — end-to-end browser, security, regression, documentation, and final
  certification.

## Pending

- None.

## Completed

- Baseline/recovery audit at `e0632460a12d3401146c12a1e79eac950a29682e`.
- M65 — IELTSAlert audit, canonical domain, persistence, permissions, and service
  foundation (9 focused tests; 113-test focused regression).
- M66 — authenticated learner workflows and platform APIs (21-test workflow/API
  regression).
- M67 — learner/reviewer frontend, backend-authoritative registry transition,
  shell integration, responsive browser journey, and centralized logout cleanup.

## Blocked

- None.

## Deferred

- External scoring providers, live test-center providers, real payments, external
  notification transport, production deployment, and migration of the separate
  `pielts` product repository.
