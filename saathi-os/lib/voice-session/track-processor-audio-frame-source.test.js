...
test("processor and reader startup failures are classified", async () => {
  const h = harness();
  class BrokenProcessor { constructor() { throw Object.assign(new Error("hidden"), { name: "NotReadableError" }); } }
  await assert.rejects(() => createTrackProcessorAudioFrameSource({ track: h.track, ProcessorImpl: BrokenProcessor }).start(), (e) => e.code === "TRACK_PROCESSOR_CONSTRUCTION_FAILED" && e.name === "NotReadableError");
  class NoReader { constructor() { this.readable = {}; } }
  await assert.rejects(() => createTrackProcessorAudioFrameSource({ track: h.track, ProcessorImpl: NoReader }).start(), (e) => e.code === "READER_ACQUISITION_FAILED");
});
