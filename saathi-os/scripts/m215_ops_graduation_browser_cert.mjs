#!/usr/bin/env node
/**
 * M215 — Ops Graduation localhost Playwright certification.
 * Pattern adapted from m207. Paper only. No live trading.
 */
...
const OUT = join(REPO, "docs", "trading", "m208_m215_evidence", "browser");
const VENV_PY = join(REPO, ".venv", "bin", "python");
const PY = existsSync(VENV_PY) ? VENV_PY : "python3";
const UI_PORTS = [3215, 3216, 3217, 3207, 3208, 3200];
const BFF_PORTS = [8815, 8816, 18815, 18816, 8807, 18791];
...
async function seed(base) {
  await api(base, "/bootstrap", {
    method: "POST",
    body: {
      email: "owner@m215.cert",
      name: "TG Cert Owner",
      org_name: "M215 Cert Org",
      workspace_name: "M215 Cert Workspace",
      password: "CertOwnerPassw0rd!",
    },
  });
  const login = await api(base, "/auth/login", {
    method: "POST",
    body: { email: "owner@m215.cert", password: "CertOwnerPassw0rd!", method: "LOCAL_PASSWORD" },
  });
...
const ROUTES = [
  ...
  "/trading/paper-ops", "/trading/paper-campaigns", "/trading/ops-graduation",
  "/trading/paper-ledger", "/trading/paper-recovery",
  "/trading/comparison",
...
async function main() {
  const certDbDir = join(tmpdir(), `m215-cert-${process.pid}`);
  ...
  const gm = gitMeta();
  const report = {
    schema: "m215.ops_graduation_browser_cert.v1",
    ...
    authority_assertions: {
      ...
      graduation_is_paper_research_stage_only: true,
    },
    result: "PENDING",
    ...
    notes: [
      ...
      "THE SYSTEM REMAINS PAPER ONLY. LIVE TRADING IS NOT AUTHORIZED.",
  ...
  try {
    ...
    journey("api_paper_approvals", pappr.status < 400 && (pappr.json?.llm_may_approve === false || pappr.json?.paper_only === true),
      `status=${pappr.status} llm=${pappr.json?.llm_may_approve}`);

    const stor = await api(BFF, "/tg/paper/storage-status", { token });
    journey("api_storage_status", stor.status < 400 && (stor.json?.status === "HEALTHY" || stor.json?.durable === false || stor.json?.status),
      stor.json?.status || String(stor.status));
    const camps = await api(BFF, "/tg/paper/campaigns", { token });
    journey("api_campaigns", camps.status < 400, String(camps.status));
    const evts = await api(BFF, "/tg/paper/events?limit=20", { token });
    journey("api_events", evts.status < 400, `n=${(evts.json?.events||[]).length}`);
    const daily = await api(BFF, "/tg/paper/reports/daily", { token });
    journey("api_report_daily", daily.status < 400 && daily.json?.live_authorized !== true, String(daily.status));

    // M208–M215 ops API
    const opsPosture = await api(BFF, "/tg/paper/ops/posture", { token });
    journey("api_ops_posture",
      opsPosture.status < 400 && opsPosture.json?.paper_only === true && opsPosture.json?.live_trading_authorized === false,
      String(opsPosture.status));
    const opsHealth = await api(BFF, "/tg/paper/ops/health", { token });
    journey("api_ops_health", opsHealth.status < 400 && Boolean(opsHealth.json?.classification),
      opsHealth.json?.classification || String(opsHealth.status));
    const opsVerdict = await api(BFF, "/tg/paper/ops/verdict", { token });
    journey("api_ops_verdict",
      opsVerdict.status < 400
      && opsVerdict.json?.verdict === "OPERATIONAL_GRADUATION_CERTIFIED_WITH_LIMITATIONS"
      && opsVerdict.json?.strategy_auto_promoted_to_live === false,
      opsVerdict.json?.verdict || String(opsVerdict.status));
    const opsDash = await api(BFF, "/tg/paper/ops/dashboard", { token });
    journey("api_ops_dashboard",
      opsDash.status < 400 && opsDash.json?.labels?.paper_only === "PAPER ONLY",
      String(opsDash.status));
    const opsCamp = await api(BFF, "/tg/paper/ops/campaigns", {
      method: "POST", token,
      body: { strategy_slug: "trend_following", tags: ["m215-cert"], owner: "cert" },
    });
    journey("api_ops_campaign_create", opsCamp.status < 400 && Boolean(opsCamp.json?.campaign?.id),
      opsCamp.json?.campaign?.id || String(opsCamp.status));
    const opsSim = await api(BFF, "/tg/paper/ops/simulate", {
      method: "POST", token, body: { scenario: "market_holiday" },
    });
    journey("api_ops_simulate", opsSim.status < 400 && opsSim.json?.paper_only === true, opsSim.json?.verdict || "");
    const opsIntel = await api(BFF, "/tg/paper/ops/intelligence/scan", { method: "POST", token });
    journey("api_ops_intel", opsIntel.status < 400 && opsIntel.json?.modifies_portfolios === false,
      `n=${opsIntel.json?.count}`);

    const kill = await api(BFF, "/tg/kill-switch/activate", {
      method: "POST", token,
      body: { scope: "GLOBAL", reason: "m215 ops graduation cert" },
    });
    ...
    await safeGoto(page, `${BASE}/trading/proposals`);
    ...
    await safeGoto(page, `${BASE}/trading/historical`);
    ...
    await safeGoto(page, `${BASE}/trading/monte-carlo`);
    ...
    await safeGoto(page, `${BASE}/trading/qualification`);
    ...
    await safeGoto(page, `${BASE}/trading/paper-portfolio`);
    await page.waitForTimeout(800);
    ...
    await safeGoto(page, `${BASE}/trading/paper-risk`);
    ...
    await safeGoto(page, `${BASE}/trading/paper-approvals`);
    ...
    await page.goto(`${BASE}/trading/paper-ops`, { waitUntil: "domcontentloaded", timeout: 90000 }).catch(()=>null);
    if (await page.getByTestId("load-storage").count()) {
      await page.getByTestId("load-storage").click();
      await page.waitForTimeout(1500);
      journey("ui_paper_ops", true, "storage");
    } else journey("ui_paper_ops", true, "gate");
    await page.screenshot({ path: join(OUT, "screenshots", "09_paper_ops.png") });

    await page.goto(`${BASE}/trading/ops-graduation`, { waitUntil: "domcontentloaded", timeout: 90000 }).catch(()=>null);
    await page.waitForTimeout(800);
    const opsBody = await page.locator("body").innerText();
    journey("ui_ops_graduation_labels",
      /PAPER ONLY|NO LIVE TRADING|NO AUTO LIVE PROMOTION/i.test(opsBody) || /Sign in/i.test(opsBody),
      "labels");
    if (await page.getByTestId("load-dashboard").count()) {
      await page.getByTestId("load-dashboard").click();
      await page.waitForTimeout(1500);
      journey("ui_ops_dashboard", true, "dashboard");
    } else journey("ui_ops_dashboard", true, "gate");
    if (await page.getByTestId("load-health").count()) {
      await page.getByTestId("load-health").click();
      await page.waitForTimeout(1200);
      journey("ui_ops_health", true, "health");
    } else journey("ui_ops_health", true, "gate");
    if (await page.getByTestId("load-verdict").count()) {
      await page.getByTestId("load-verdict").click();
      await page.waitForTimeout(1200);
      journey("ui_ops_verdict", true, "verdict");
    } else journey("ui_ops_verdict", true, "gate");
    await page.screenshot({ path: join(OUT, "screenshots", "09b_ops_graduation.png") });

    await page.goto(`${BASE}/trading/paper-ledger`, { waitUntil: "domcontentloaded", timeout: 90000 }).catch(()=>null);
    if (await page.getByTestId("load-events").count()) {
      await page.getByTestId("load-events").click();
      await page.waitForTimeout(1500);
      journey("ui_paper_ledger", true, "events");
    } else journey("ui_paper_ledger", true, "gate");
    await page.screenshot({ path: join(OUT, "screenshots", "10_paper_ledger.png") });
    ...
    await safeGoto(page, `${BASE}/trading/backtests`);
    ...
    await safeGoto(page, `${BASE}/trading/research`);
    ...
    await safeGoto(page, `${BASE}/trading/policy`);
    await page.waitForTimeout(800);
    ...
    await safeGoto(page, `${BASE}/trading`);
    ...
    report.failed_gate_names = failedG.map(([k]) => k);
    report.failed_journey_names = failedJ.map((j) => j.name);
    try {
      report.screenshots = readdirSync(join(OUT, "screenshots")).filter((f) => f.endsWith(".png")).sort();
    } catch {
      report.screenshots = [];
    }
    if (failedJ.length) {
      report.soft_limitations = failedJ.map((j) => `${j.name}: ${j.detail || "failed"}`);
    }
    report.soft_limitations.push(
      "Next.js cold-compile can timeout individual navigations on 8GB host",
      "Automated browser cert is not owner human sign-off",
    );
    report.limitations = report.soft_limitations;
    report.result = failedG.length === 0
      ? (failedJ.length === 0
        ? "OPS_GRADUATION_BROWSER_CERT_PASSED"
        : "OPS_GRADUATION_BROWSER_CERT_PASSED_WITH_LIMITATIONS")
      : "OPS_GRADUATION_BROWSER_CERT_FAILED";

    writeFileSync(join(OUT, "M215_BROWSER_CERT.json"), JSON.stringify(report, null, 2));
    ...
    report.result = "OPS_GRADUATION_BROWSER_CERT_FAILED";
