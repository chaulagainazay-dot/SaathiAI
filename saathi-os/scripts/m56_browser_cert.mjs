...
const OUT = join(REPO, "docs", "platform", "m56_evidence");
const ALLOW_LIMITATIONS = process.argv.includes("--allow-limitations");
const USE_DEV = process.env.M56_BUILD !== "1";
const VENV_PY = join(REPO, ".venv", "bin", "python");
const PY = existsSync(VENV_PY) ? VENV_PY : "python3";

const UI_PORTS = [3160, 3162, 3164].map(Number);
const BFF_PORTS = [8800, 18800, 18802].map(Number);
...
async function main() {
  ...
  report.verdict = hardOk
    ? "M56_BROWSER_CERTIFIED"
    : ALLOW_LIMITATIONS
      ? "M56_BROWSER_CERTIFIED_WITH_LIMITATIONS"
      : "M56_BROWSER_CERT_FAILED";
  writeFileSync(join(OUT, "m56_browser_cert.json"), JSON.stringify(report, null, 2));
  const failed = Object.entries(report.hardGates)
    .filter(([, g]) => !g.ok)
    .map(([k]) => k);
  console.log(`[m56] verdict=${report.verdict} failedHardGates=${failed.join(",") || "none"}`);
  console.log(`[m56] evidence: ${join(OUT, "m56_browser_cert.json")}`);
  if (!hardOk && !ALLOW_LIMITATIONS) process.exit(1);
}

main().catch((e) => {
  console.error("[m56] fatal", e);
  process.exit(1);
});
