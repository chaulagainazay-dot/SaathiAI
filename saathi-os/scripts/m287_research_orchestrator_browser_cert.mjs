#!/usr/bin/env node
/**
 * M287 — Autonomous Research Orchestrator browser cert.
...
const OUT = join(REPO, "docs", "trading", "m280_m287_evidence", "browser");
const EVIDENCE = join(REPO, "docs", "trading", "m280_m287_evidence");
...
async function seed(base) {
  await api(base, "/bootstrap", {
    method: "POST",
    body: {
      email: "owner@m287.cert",
      name: "RO Cert Owner",
      org_name: "M287 Cert Org",
      workspace_name: "M287 Cert Workspace",
      password: "CertOwnerPassw0rd!",
    },
  });
  const login = await api(base, "/auth/login", {
    method: "POST",
    body: { email: "owner@m287.cert", password: "CertOwnerPassw0rd!", method: "LOCAL_PASSWORD" },
  });
...
async function main() {
  const certDbDir = join(tmpdir(), `m287-cert-${process.pid}`);
  ...
  const gm = gitMeta();
  const report = {
    schema: "m287.research_orchestrator_browser_cert.v1",
    verdict: "PENDING",
    branch: gm.branch,
    sha: gm.sha,
  ...
  try {
    ...
    const dash = await api(BFF, "/tg/research-orchestrator/dashboard", { token });
    ...
    const boot = await api(BFF, "/tg/research-orchestrator/bootstrap", { method: "POST", token });
    journey("bootstrap_pipeline", boot.json?.ok === true && (boot.json?.ran || []).length >= 1, boot.json?.session_id || "");
    if (boot.json?.ok) pass("bootstrap", boot.json.session_id); else fail("bootstrap", boot.text?.slice(0, 200));

    const jobs = await api(BFF, "/tg/research-orchestrator/jobs", { token });
    journey("job_queue", (jobs.json?.count || 0) >= 1, String(jobs.json?.count));

    const workers = await api(BFF, "/tg/research-orchestrator/workers", { token });
    journey("workers", (workers.json?.max_workers || 0) >= 1, String(workers.json?.max_workers));

    const budget = await api(BFF, "/tg/research-orchestrator/budget", { token });
    journey("budget", budget.json?.total_units > 0, String(budget.json?.remaining_units));

    const templates = await api(BFF, "/tg/research-orchestrator/templates", { token });
    journey("templates", (templates.json?.count || 0) >= 1, String(templates.json?.count));

    const notebook = await api(BFF, "/tg/research-orchestrator/notebook", { token });
    journey("notebook", Boolean(notebook.json?.title), notebook.json?.title || "");

    const strategies = await api(BFF, "/tg/research-orchestrator/strategies", { token });
    journey("strategy_registry_v2", (strategies.json?.count || 0) >= 1, String(strategies.json?.count));

    const broker = await api(BFF, "/tg/research-orchestrator/broker/connect", { method: "POST", token });
    ...
    const cred = await api(BFF, "/tg/research-orchestrator/credentials", {
    ...
    const order = await api(BFF, "/tg/research-orchestrator/orders", { method: "POST", token });
    ...
    const canary = await api(BFF, "/tg/research-orchestrator/canary/activate", { method: "POST", token });
    ...
    const paperExec = await api(BFF, "/tg/research-orchestrator/paper-execution/activate", { method: "POST", token });
    ...
    const cert = await api(BFF, "/tg/research-orchestrator/certify", { method: "POST", token });
    journey("certify",
      (cert.json?.verdict || "").includes("AUTONOMOUS_RESEARCH_ORCHESTRATOR"),
      cert.json?.verdict || "");
    ...
    await safeGoto(page, `${BASE}/trading/research-orchestrator`);
    await page.waitForTimeout(1200);
    const shot = join(OUT, "screenshots", "research_orchestrator_control_center.png");
    ...
    try {
      ...
      const shot2 = join(OUT, "screenshots", "research_orchestrator_after_bootstrap.png");
    ...
    const labelsVisible =
      ...
      || bodyText.includes("Autonomous Research Orchestrator")
    ...
    for (const tid of [
      "load-dashboard", "load-verdict", "run-bootstrap", "load-jobs",
      "load-workers", "load-budget", "load-templates", "load-notebook",
      "load-failures", "load-calendar", "load-evidence",
      "refuse-broker", "refuse-credentials", "refuse-orders",
    ]) {
    ...
    const shot2 = join(OUT, "screenshots", "research_orchestrator_after_actions.png");
    await page.screenshot({ path: shot2, fullPage: true }).catch(() => null);
    report.screenshots.push(shot2);

    const after = await page.content();
    journey("queue_or_orchestrator_visible",
      after.includes("Orchestrator") || after.includes("Job") || after.includes("Queue") || appGate,
      appGate ? "gated" : "checked");
    journey("deterministic_label",
      after.includes("DETERMINISTIC") || labelsVisible || appGate,
      "");
    ...
    const sec = await api(BFF, "/tg/research-orchestrator/security", { token });
  ...
  report.verdict = hardOk
    ? "AUTONOMOUS_RESEARCH_ORCHESTRATOR_BROWSER_CERT_PASSED_WITH_LIMITATIONS"
    : "M287_BROWSER_CERT_FAILED";
  report.hard_gates_pass = hardOk;
  report.journeys_pass = journeyOk;

  writeFileSync(join(OUT, "M287_BROWSER_CERT.json"), JSON.stringify(report, null, 2));
  writeFileSync(join(EVIDENCE, "M287_BROWSER_CERT_SUMMARY.json"), JSON.stringify({
