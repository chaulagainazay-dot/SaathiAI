...
const OUT = join(REPO, "docs", "trading", "m200_m207_evidence", "browser");
const VENV_PY = join(REPO, ".venv", "bin", "python");
const PY = existsSync(VENV_PY) ? VENV_PY : "python3";
const UI_PORTS = [3207, 3208, 3209, 3191, 3192, 3193, 3200];
const BFF_PORTS = [8807, 8808, 18807, 18808, 8791, 18791];
...
async function seed(base) {
  await api(base, "/bootstrap", {
    method: "POST",
    body: {
      email: "owner@m207.cert",
      name: "TG Cert Owner",
      org_name: "M207 Cert Org",
      workspace_name: "M207 Cert Workspace",
      password: "CertOwnerPassw0rd!",
    },
  });
  const login = await api(base, "/auth/login", {
    method: "POST",
    body: { email: "owner@m207.cert", password: "CertOwnerPassw0rd!", method: "LOCAL_PASSWORD" },
  });
...
const ROUTES = [
  ...
  "/trading/paper-ops", "/trading/paper-campaigns", "/trading/paper-ledger", "/trading/paper-recovery",
  "/trading/comparison",
...
async function main() {
  const certDbDir = join(tmpdir(), `m207-cert-${process.pid}`);
  ...
  const report = {
    schema: "m207.durable_paper_browser_cert.v1",
  ...
  try {
    ...
    const kill = await api(BFF, "/tg/kill-switch/activate", {
      method: "POST", token,
      body: { scope: "GLOBAL", reason: "m207 durable paper cert" },
    });
    ...
    report.result = failedG.length === 0
      ? (failedJ.length === 0
        ? "DURABLE_PAPER_BROWSER_CERT_PASSED"
        : "DURABLE_PAPER_BROWSER_CERT_PASSED_WITH_LIMITATIONS")
      : "DURABLE_PAPER_BROWSER_CERT_FAILED";

    writeFileSync(join(OUT, "M207_BROWSER_CERT.json"), JSON.stringify(report, null, 2));
    ...
    report.result = "DURABLE_PAPER_BROWSER_CERT_FAILED";
