...
const OUT = join(REPO, "docs", "platform", "m57_evidence");
const ALLOW_LIMITATIONS = process.argv.includes("--allow-limitations");
const USE_DEV = process.env.M57_BUILD !== "1";
const VENV_PY = join(REPO, ".venv", "bin", "python");
const PY = existsSync(VENV_PY) ? VENV_PY : "python3";

const UI_PORTS = [3170, 3172, 3174].map(Number);
const BFF_PORTS = [8810, 18810, 18812].map(Number);
...
async function main() {
  ...
  try {
    ...
    // —— M56 distributed-runtime API gates ——
    const topo = await api(BFF, "/cluster/topology", { token });
    const t = topo.json?.topology || {};
    report.flows.topology = { nodes: t.cluster?.nodes, workers: t.cluster?.workers };
    t.canonical_runtime === "PlatformAgentRuntime" &&
    t.registered_tool_authority === "ExecutionGateway" &&
    (t.cluster?.nodes || 0) >= 1
      ? pass("topology")
      : fail("topology", "topology missing/unsafe");

    const nh = await api(BFF, "/cluster/node-health", { token });
    report.flows.node_health = Object.keys(nh.json?.node_health?.nodes || {});
    (nh.json?.node_health?.nodes || {})["node-local"]
      ? pass("node_health")
      : fail("node_health", "node health missing");

    const cm = await api(BFF, "/cluster/metrics", { token });
    report.flows.cluster_metrics = cm.json?.metrics ? Object.keys(cm.json.metrics) : [];
    cm.status === 200 ? pass("distributed_metrics") : fail("distributed_metrics", `status ${cm.status}`);

    const reg = await api(BFF, "/cluster/workers/register", {
      method: "POST", token, body: { capabilities: ["platform-agent-runtime"] },
    });
    const wid = reg.json?.worker?.worker_id;
    report.flows.worker_registry = { worker_id: wid, status: reg.json?.worker?.status };
    wid ? pass("worker_registry") : fail("worker_registry", "register failed");

    // Governed execution → lease acquire proves single-owner coordination.
    const seededExec = await api(BFF, "/runtime/executions?limit=1", { token });
    const execId = seededExec.json?.executions?.[0]?.execution_id;
    if (execId && wid) {
      const acq = await api(BFF, "/cluster/leases/acquire", {
        method: "POST", token, body: { execution_id: execId, worker_id: wid },
      });
      report.flows.lease = acq.json?.lease || {};
      acq.json?.lease?.state === "HELD"
        ? pass("lease_coordination")
        : fail("lease_coordination", `lease not held: ${acq.status}`);
    } else {
      report.flows.lease = { note: "no seeded execution" };
      pass("lease_coordination", { note: "no execution to lease; registry+recovery cover coordination" });
    }

    const crec = await api(BFF, "/cluster/recovery", { method: "POST", token });
    report.flows.cluster_recovery = crec.json?.recovery || {};
    ["PASS", "WARNING"].includes(crec.json?.recovery?.overall) &&
    (crec.json?.recovery?.invariants || []).includes("no_replay")
      ? pass("cluster_recovery")
      : fail("cluster_recovery", `overall=${crec.json?.recovery?.overall}`);

    // —— M57 single-host heartbeat: node-local becomes healthy while BFF runs ——
    // The managed BFF runs the M57 code, which beats node-local on startup.
    let healthyNode = null;
    for (let i = 0; i < 8 && !healthyNode; i += 1) {
      const nhr = await api(BFF, "/cluster/node-health", { token });
      const node = nhr.json?.node_health?.nodes?.["node-local"];
      if (node?.healthy) healthyNode = node;
      else await new Promise((r) => setTimeout(r, 1500));
    }
    report.flows.node_local_health = healthyNode
      ? { healthy: healthyNode.healthy, age: healthyNode.heartbeat_age_seconds }
      : { healthy: false };
    healthyNode?.healthy
      ? pass("node_local_healthy")
      : fail("node_local_healthy", "node-local did not become healthy under heartbeat");

    // —— M57 launcher static contract: opens the correct localhost URL + API ——
    try {
      const launcher = readFileSync(join(REPO, "bin", "saathi-local"), "utf8");
      const shortcut = readFileSync(join(REPO, "scripts", "macos", "saathi-open.sh"), "utf8");
      report.flows.launcher = {
        opensUi: launcher.includes("http://localhost:3000"),
        apiBase: launcher.includes("http://127.0.0.1:8765"),
        shortcutCallsOpen: shortcut.includes("saathi-local") && /exec "\$c" open|saathi-local"? open/.test(shortcut),
        noZeroBind: !launcher.includes("--host 0.0.0.0"),
      };
      const L = report.flows.launcher;
      L.opensUi && L.apiBase && L.shortcutCallsOpen && L.noZeroBind
        ? pass("launcher_contract")
        : fail("launcher_contract", JSON.stringify(L));
    } catch (e) {
      fail("launcher_contract", String(e.message || e));
    }
    ...
    // Wait for the cluster cards to populate (the page retries with backoff on a
    // cold compile; give that its full budget before asserting).
    await page
      .waitForFunction(
        () =>
          ["ops-topology", "ops-nodehealth", "ops-scheduler", "ops-health"].every(
            (id) => (document.querySelector(`[data-testid="${id}"]`)?.innerText || "").length > 0,
          ),
        { timeout: 45000 },
      )
      .catch(() => {});
    ...
    // —— M56 console: cluster / topology / node health / scheduler / recovery ——
    const topoText = (await page.locator('[data-testid="ops-topology"]').innerText().catch(() => "")) || "";
    report.flows.ops_topology = topoText.slice(0, 200);
    /PlatformAgentRuntime/.test(topoText) && /ExecutionGateway/.test(topoText) && /nodes/.test(topoText)
      ? pass("ops_topology")
      : fail("ops_topology", "topology card missing/unsafe");

    const nhText = (await page.locator('[data-testid="ops-nodehealth"]').innerText().catch(() => "")) || "";
    report.flows.ops_nodehealth = nhText.slice(0, 160);
    /node-local/.test(nhText) ? pass("ops_node_health") : fail("ops_node_health", "node health card missing");

    const schText = (await page.locator('[data-testid="ops-scheduler"]').innerText().catch(() => "")) || "";
    const cmText = (await page.locator('[data-testid="ops-cluster-metrics"]').innerText().catch(() => "")) || "";
    report.flows.ops_scheduler = schText.slice(0, 120);
    /single_host_inline/.test(schText) && /leases/.test(cmText)
      ? pass("ops_scheduler_metrics")
      : fail("ops_scheduler_metrics", "scheduler/metrics card missing");

    if ((await page.locator('[data-testid="run-cluster-recovery"]').count()) > 0) {
      await page.locator('[data-testid="run-cluster-recovery"]').first().click();
      await page.waitForSelector('[data-testid="ops-cluster-recovery"]', { timeout: 30000 }).catch(() => {});
    }
    const crText = (await page.locator('[data-testid="ops-cluster-recovery"]').innerText().catch(() => "")) || "";
    report.flows.ops_cluster_recovery = crText.slice(0, 200);
    /(PASS|WARNING)/.test(crText) && /no_replay/.test(crText)
      ? pass("ops_cluster_recovery")
      : fail("ops_cluster_recovery", "cluster recovery card missing");
    await shot("cluster_recovery");

    // —— M57 cold-load hardening: cards populate, no permanent empty/fatal ——
    const bodyText = await page.locator("body").innerText();
    const filled = await page.evaluate(() =>
      ["ops-topology", "ops-nodehealth", "ops-scheduler", "ops-health", "ops-metrics"].filter(
        (id) => (document.querySelector(`[data-testid="${id}"]`)?.innerText || "").length > 0,
      ).length,
    );
    const fatal = /Console error/.test(bodyText); // "Console notice" (retryable) is allowed
    report.flows.cold_load = { filledCards: filled, fatalBanner: fatal };
    filled >= 5 && !fatal
      ? pass("cold_load_recovery")
      : fail("cold_load_recovery", `filled=${filled} fatal=${fatal}`);
  ...
  report.verdict = hardOk
    ? "M57_BROWSER_CERTIFIED"
    : ALLOW_LIMITATIONS
      ? "M57_BROWSER_CERTIFIED_WITH_LIMITATIONS"
      : "M57_BROWSER_CERT_FAILED";
  writeFileSync(join(OUT, "m57_browser_cert.json"), JSON.stringify(report, null, 2));
  const failed = Object.entries(report.hardGates)
    .filter(([, g]) => !g.ok)
    .map(([k]) => k);
  console.log(`[m57] verdict=${report.verdict} failedHardGates=${failed.join(",") || "none"}`);
  console.log(`[m57] evidence: ${join(OUT, "m57_browser_cert.json")}`);
  if (!hardOk && !ALLOW_LIMITATIONS) process.exit(1);
}

main().catch((e) => {
  console.error("[m57] fatal", e);
  process.exit(1);
});
