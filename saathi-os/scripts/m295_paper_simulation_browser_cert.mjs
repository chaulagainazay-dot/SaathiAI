#!/usr/bin/env node
/**
 * M295 — Institutional Paper Simulation browser cert.
...
const OUT = join(REPO, "docs", "trading", "m288_m295_evidence", "browser");
const EVIDENCE = join(REPO, "docs", "trading", "m288_m295_evidence");
...
async function seed(base) {
  await api(base, "/bootstrap", {
    method: "POST",
    body: {
      email: "owner@m295.cert",
      name: "PS Cert Owner",
      org_name: "M295 Cert Org",
      workspace_name: "M295 Cert Workspace",
      password: "CertOwnerPassw0rd!",
    },
  });
  const login = await api(base, "/auth/login", {
    method: "POST",
    body: { email: "owner@m295.cert", password: "CertOwnerPassw0rd!", method: "LOCAL_PASSWORD" },
  });
...
async function main() {
  const certDbDir = join(tmpdir(), `m295-cert-${process.pid}`);
  ...
  const gm = gitMeta();
  const report = {
    schema: "m295.paper_simulation_browser_cert.v1",
    verdict: "PENDING",
    branch: gm.branch,
    sha: gm.sha,
  ...
  try {
    ...
    const dash = await api(BFF, "/tg/paper-simulation/dashboard", { token });
    ...
    const boot = await api(BFF, "/tg/paper-simulation/bootstrap", { method: "POST", token });
    journey("bootstrap_pipeline", boot.json?.ok === true && boot.json?.market_fill === true, boot.json?.portfolio_id || "");
    if (boot.json?.ok) pass("bootstrap", boot.json.portfolio_id); else fail("bootstrap", boot.text?.slice(0, 200));

    const exchange = await api(BFF, "/tg/paper-simulation/exchange", { token });
    journey("virtual_exchange", exchange.json?.exchange === "VIRTUAL_PAPER_EXCHANGE" && exchange.json?.real_exchange === false, exchange.json?.exchange || "");
    if (exchange.json?.real_exchange === false) pass("virtual_exchange", "ok"); else fail("virtual_exchange", "real");

    const book = await api(BFF, "/tg/paper-simulation/order-book/SPY", { token });
    journey("order_book", (book.json?.bids || []).length >= 1, String(book.json?.bids?.length));

    const pid = boot.json?.portfolio_id;
    if (pid) {
      const fills = await api(BFF, `/tg/paper-simulation/portfolios/${pid}/fills`, { token });
      journey("fill_audit", (fills.json?.count || 0) >= 1, String(fills.json?.count));
      const cash = await api(BFF, `/tg/paper-simulation/portfolios/${pid}/cash`, { token });
      journey("cash_ledger", (cash.json?.count || 0) >= 1, String(cash.json?.count));
    }

    const ks = await api(BFF, "/tg/paper-simulation/kill-switch", { token });
    journey("kill_switch", ks.status < 400, ks.json?.state || "");

    const realOrd = await api(BFF, "/tg/paper-simulation/real-orders", { method: "POST", token });
    journey("real_orders_refused", realOrd.json?.ok === false, realOrd.json?.code || "");
    if (realOrd.json?.ok === false) pass("real_orders_refused", realOrd.json?.code); else fail("real_orders_refused", "allowed");

    const live = await api(BFF, "/tg/paper-simulation/live/activate", { method: "POST", token });
    journey("live_refused", live.json?.ok === false, live.json?.code || "");
    if (live.json?.ok === false) pass("live_refused", live.json?.code); else fail("live_refused", "allowed");

const broker = await api(BFF, "/tg/paper-simulation/broker/connect", { method: "POST", token });
    ...
    const cred = await api(BFF, "/tg/paper-simulation/credentials", {
    ...
    const canary = await api(BFF, "/tg/paper-simulation/canary/activate", { method: "POST", token });
    ...
    const cert = await api(BFF, "/tg/paper-simulation/certify", { method: "POST", token });
    journey("certify",
      (cert.json?.verdict || "").includes("INSTITUTIONAL_PAPER_TRADING_SIMULATION") && cert.json?.ok === true,
      cert.json?.verdict || "");
    if (cert.json?.ok && String(cert.json?.verdict || "").includes("INSTITUTIONAL_PAPER_TRADING_SIMULATION")) {
    ...
    await safeGoto(page, `${BASE}/trading/paper-simulation`);
    await page.waitForTimeout(1200);
    const shot = join(OUT, "screenshots", "paper_simulation_control_center.png");
    ...
    try {
      ...
      const shot2 = join(OUT, "screenshots", "paper_simulation_after_bootstrap.png");
    ...
    const labelsVisible =
      ...
      || bodyText.includes("Institutional Paper Trading Simulation")
    ...
    for (const tid of [
      "load-dashboard", "load-verdict", "run-bootstrap", "load-exchange",
      "load-book", "load-kill-switch", "load-calendar",
      "refuse-broker", "refuse-credentials", "refuse-real-orders", "refuse-live",
    ]) {
    ...
    const shot2 = join(OUT, "screenshots", "paper_simulation_after_actions.png");
    await page.screenshot({ path: shot2, fullPage: true }).catch(() => null);
    report.screenshots.push(shot2);

    const after = await page.content();
    journey("paper_sim_visible",
      after.includes("Paper") || after.includes("Simulation") || after.includes("VIRTUAL") || appGate,
      appGate ? "gated" : "checked");
    journey("virtual_exchange_label",
      after.includes("VIRTUAL EXCHANGE") || labelsVisible || appGate,
      "");
    ...
    const sec = await api(BFF, "/tg/paper-simulation/security", { token });
  ...
  report.verdict = hardOk
    ? "INSTITUTIONAL_PAPER_SIMULATION_BROWSER_CERT_PASSED_WITH_LIMITATIONS"
    : "M295_BROWSER_CERT_FAILED";
  report.hard_gates_pass = hardOk;
  report.journeys_pass = journeyOk;

  writeFileSync(join(OUT, "M295_BROWSER_CERT.json"), JSON.stringify(report, null, 2));
  writeFileSync(join(EVIDENCE, "M295_BROWSER_CERT_SUMMARY.json"), JSON.stringify({
