...
const OUT = join(REPO, "docs", "trading", "m184_m191_evidence", "browser");
...
async function seed(base) {
  await api(base, "/bootstrap", {
    method: "POST",
    body: {
      email: "owner@m191.cert",
      name: "TG Cert Owner",
      org_name: "M191 Cert Org",
      workspace_name: "M191 Cert Workspace",
      password: "CertOwnerPassw0rd!",
    },
  });
  const login = await api(base, "/auth/login", {
    method: "POST",
    body: { email: "owner@m191.cert", password: "CertOwnerPassw0rd!", method: "LOCAL_PASSWORD" },
  });
...
async function main() {
  const certDbDir = join(tmpdir(), `m191-cert-${process.pid}`);
  ...
  const report = {
    schema: "m191.trading_guardian_historical_browser_cert.v1",
  ...
  try {
    ...
    const kill = await api(BFF, "/tg/kill-switch/activate", {
      method: "POST", token,
      body: { scope: "GLOBAL", reason: "m191 browser cert" },
    });
    ...
    report.result = failedG.length === 0
      ? (failedJ.length === 0
        ? "TRADING_GUARDIAN_HISTORICAL_BROWSER_CERT_PASSED"
        : "TRADING_GUARDIAN_HISTORICAL_BROWSER_CERT_PASSED_WITH_LIMITATIONS")
      : "TRADING_GUARDIAN_HISTORICAL_BROWSER_CERT_FAILED";

    writeFileSync(join(OUT, "M191_BROWSER_CERT.json"), JSON.stringify(report, null, 2));
    ...
    report.result = "TRADING_GUARDIAN_HISTORICAL_BROWSER_CERT_FAILED";
