...
- ⬜ Dependency-ordered next slices (Ajay's graph): Portfolio Intelligence (cash/sector/correlation/concentration/drawdown/liquidity/rebalancing — evaluates *approved recommendations*, not raw opportunities) → **Portfolio Impact Simulator** (before/after effect of adding a position) → Execution Connectors (post-L4-approval only) → Investment Learning → Financial Mission Control (the *consumer*, built last) → broker integrations (Alpaca/Binance). Tag `v0.4.0-finance` when complete.
