...
- ⬜ Dependency-ordered next slices (Ajay's graph): Execution Connectors (post-L4-approval only, no Governance bypass) → Investment Learning (episodes → governed knowledge via M2 Promotion Engine) → Financial Mission Control (the *consumer*, built last) → broker integrations (Alpaca/Binance). Tag `v0.4.0-finance` when complete.
