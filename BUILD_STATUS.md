...
- ⬜ Remaining to close M5: Executive Intelligence integration (surface finance actions/insights in the existing morning briefing) → **M4.5-style Integration Sprint** (verify every finance component emits standardized events, episodes flow to the shared Learning Runtime, KPIs registered, L4 governance + approval holds on every investment action, reconcile Brain.md/Wisdom.md) → tag `v0.4.0-finance`.
