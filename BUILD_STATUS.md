...
- ⬜ Dependency-ordered next slices (Ajay's graph): Trade Journal Engine (auto-records opportunity→research→DD→sim→risk→allocation→approval→execution→P/L→lessons per trade) → Investment Learning Runtime (episodes → governed knowledge via M2 Promotion Engine) → Financial Mission Control (the *consumer*, built last) → broker integrations (Alpaca/Binance). Tag `v0.4.0-finance` when complete.
