...
- ⬜ Dependency-ordered next slices (Ajay's graph): Investment Learning Runtime (consumes *journals*, not raw executions → governed knowledge via M2 Promotion Engine → Knowledge Graph → Publication → Executive Intelligence) → Financial Mission Control (the *consumer*, built last) → broker integrations (Alpaca/Binance). Tag `v0.4.0-finance` when complete.
