...
- ⬜ Dependency-ordered next slices (Ajay's graph): wire Due Diligence/Simulation/Risk/Recommendation to consume Opportunities → Portfolio Intelligence (concentration/correlation/cash) → Execution Connectors (post-L4-approval only) → Investment Learning → Financial Mission Control (the *consumer*, built last) → broker integrations (Alpaca/Binance). Tag `v0.4.0-finance` when complete.
