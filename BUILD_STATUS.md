...
- ✅ **Research Department** (`saathi/research.py`, 9 tests) — the shared evidence source every downstream capability consumes (AP-01, build-once-reuse-everywhere). Seven specialized agents (Market, On-Chain, Project, News, Sentiment, Fundamental, Technical) emit categorized `Evidence`; deterministic core (AP-17) with injected collectors/clock/publisher/recorder (AP-12). **Flagship: Research Confidence Framework** — transparent weighted breakdown (Fundamentals 30% · On-chain 20% · Technical 15% · Team&GitHub 15% · Sentiment 10% · Risk 10%) + missing-evidence, cross-source contradiction detection, freshness horizon, and independent-source count, so every recommendation is explainable and auditable (AP-15→20). Guardrails encoded: **sentiment is never a sole basis** (sentiment/news/market-only research is not actionable) and technical complements — never replaces — fundamentals. Each research pass becomes a Learning Runtime episode.
- ✅ **Opportunity Intelligence Department** (`saathi/opportunity.py`, 7 tests) — *universal*, not just investing: every discovered thing (crypto IDO, supplier, open-source model, affiliate, trending IELTS topic) becomes ONE standardized `Opportunity` the whole AI-OS consumes (AP-01). Pipeline: **Scan → Normalize → Research (via Research Dept) → Confidence → Deduplicate → Rank**. Discovery agents (Crypto/Stock/AI/Business/Revenue) are thin category + injected scanner. **Cross-agent dedup** collapses "GPT-X released / announced / available" into one merged opportunity (strongest estimates kept, sources unioned). Score = ROI × Confidence × Strategic Fit × Urgency × Evidence Quality / Risk (Dream Engine can later tune strategic_fit). Sentiment-only opportunities are rejected (inherits Research guardrail). **Flagship: Opportunity Memory** (SQLite) — never forget: rejected opportunities are monitored and *resurrected* when a re-observation scores materially higher (first_seen preserved). Each discovery cycle is a Learning Runtime episode.
- ✅ **Discovery → Recommendation Bridge** (`saathi/investment_pipeline.py`, 6 tests) — the orchestration layer that turns the eight independent engines into ONE traceable flow: discover → research → validate → simulate → size → recommend → capital-allocate → [Governance L4 human approval] → learn. No stage knows the next (clean boundaries). Every opportunity accumulates into a single **`InvestmentCase`** — the complete, explainable evidence package (opportunity + research + DD + simulation + risk + allocation + recommendation + approval + evidence_ids) that downstream departments receive instead of ten separate objects. Human-approval spine preserved end-to-end: recommendations are L4, always `requires_approval`, `.executable` only after `case.approve()`; DD failure → REJECT (ranked out, cannot approve); research contradictions → HOLD; capital allocation runs across all BUYs so every dollar competes. Each run is a Learning Runtime episode.
- ✅ **Portfolio Intelligence Department** (`saathi/portfolio.py`, 11 tests) — wealth as a system, six deterministic engines over one source of truth: **Portfolio Registry** (owns cash/positions/transactions; only it mutates holdings), **Allocation Engine** (cash/crypto/stocks/business/AI %, all USD-normalized), **Diversification Engine** (sector/country/asset-class HHI + stablecoin/meme/small-cap/AI exposure → 0–100 score), **Portfolio Risk v2** (drawdown, concentration, correlation, liquidity, volatility, leverage, cash runway — portfolio-level, distinct from opportunity risk), **Portfolio Health Score** (one number from six sub-scores, for the morning briefing), **Rebalancing Engine** (recommendations only — never trades). Plus **Portfolio Impact Simulator** — evaluates an opportunity *inside* the portfolio (before vs after health/concentration/volatility/liquidity), never mutating the live registry; and **Capital Reserve Engine** — every dollar competes across financial assets AND business expansion (AI coin vs YouTube marketing vs kitchen equipment vs SaathiAI feature), respecting an emergency-reserve floor.
- ✅ **Execution Layer** (`saathi/execution.py`, 10 tests) — governed, auditable, paper-first, no bypass. **ExecutionIntent** is a frozen, replayable audit record that can ONLY be born from an `executable` (human-approved, Governance L4) InvestmentCase — `from_case` raises `PermissionError` otherwise, so no trade bypasses approval. **BrokerConnector** interface (validate/preview/execute/cancel/status/positions/balances/orders) gives broker independence — callers use `ExecutionService.execute(intent)` and never touch Binance/Alpaca directly (same abstraction as Model Router / Tool Registry). **PaperConnector** + **ReplayConnector** share the live interface (paper-first: test for months, zero capital risk). Execution requires explicit preview confirmation. **Idempotent failure recovery**: on broker timeout we query status by `intent_id` and reconcile instead of blind-retrying — a lost response can't double-trade (proven by `timeout_filled` test). On fill, the service updates the Portfolio Registry, publishes `execution.filled`/`.failed`/etc. (Mission Control / Learning / Portfolio subscribe, never poll), and records a Learning Runtime episode. **Broker Registry** selects connectors by name.
- ✅ **Investment Memory & Trade Journal** (`saathi/trade_journal.py`, 8 tests) — the financial specialization of Platform Memory: an append-only record (never deleted or rewritten) of every capital-allocation lifecycle. Each journal freezes a **Decision Snapshot** (what the system believed: opportunity score, research confidence, risk, simulation EV, allocation, portfolio health, exec recommendation) and a **Market Context Snapshot** (BTC dominance, Fear&Greed, VIX, rates, regime, news) so future analysis stays honest after algorithms change. On close it computes PnL/return/duration, records the **Exit Reason** (target/stop/manual/time/risk/strategy/rebalance), stores structured **Lessons** (Observation/Root-Cause/Recommendation/Confidence/Capabilities/Evidence — Learning-Runtime shape), and carries **Strategy / Research / Portfolio attribution**. A closed journal is immutable. **Performance Analytics** derives win-rate, expectancy, profit factor, Sharpe, max drawdown, avg holding time, best/worst strategy & asset class; **research_attribution** separates winners-vs-losers per agent so the Research Department can auto-reweight (closes the loop inside M5). Each close publishes `trade.closed` and records a Learning episode.
- ✅ **Investment Learning Runtime** (`saathi/investment_learning.py`, 8 tests) — "how do we become a better investor after every trade?" Specializes the M2 learning platform (does not reinvent it): consumes completed Trade Journals through six deterministic stages — Performance Analysis → Pattern Discovery → Strategy Eval → Research Eval → Portfolio Eval → Improvement Proposals — and emits proposals into the existing `CapabilityImprovementRegistry` (M2), which flow through Promotion → Knowledge Graph → Publication → Executive Intelligence. **Learning proposes, never mutates** (AP-18/20): proposals land as PROPOSED, never auto-applied. Computes per-agent **research predictive accuracy** (winners-vs-losers signal agreement → reweight the Research Confidence Framework), per-strategy scores (negative-expectancy → reduce-weight proposal), sentiment-only avoidance, and portfolio-health attribution. **Regime Learning** tags every trade by market regime so it learns "momentum wins in bull, fails sideways" rather than a flat win rate. Publishes `investment.learning_completed` + records an episode.
- ✅ **Financial Mission Control** (`saathi/financial_mission_control.py`, 7 tests) — the consumer/presentation layer (performs NO analysis; formatting only). Aggregates Portfolio Intelligence, Investment Pipeline, Research, Trade Journal, Investment Learning, Revenue, and Executive Intelligence into one morning briefing across eight sections: Investment Overview · Opportunity Pipeline funnel · Research Intelligence (predictive accuracy by agent) · Strategy Performance (stars) · Portfolio Health · Learning & Knowledge · **Dream Contribution by business** (AI Studio / Cafeteria / Travel / Crypto …) · CEO Action List (from Executive Intelligence). Plus **Capital Allocation Timeline** — the historical map of "where every dollar produced the greatest long-term value" across markets AND businesses (best/worst/net). `render()` for the briefing, `to_dict()` for the API; publishes `financial_mission_control.rendered`; optional injected Telegram sender. Cannot bypass governance — it executes nothing.
- ✅ **Executive Intelligence — Financial Integration** (`saathi/executive_finance.py`, 5 tests) — the single synthesizing brain now *consumes* Financial Mission Control's dashboard (no recomputation, no duplicated logic) and folds it into the CEO briefing. **Cross-Department Priority Engine** normalizes recommendations from every department (AI Studio/Discovery/Cafeteria/Travel/Investment/Learning/Storage) into ONE deterministic ranked queue where value·confidence·(1-risk) dominate, goal-alignment swings 40%, urgency only 20% — so an urgent low-value task can't displace a strategic high-value one. **CEO Question Framework** guarantees the briefing answers the eight cockpit questions (what happened / what to do / blockers / made money / lost money / what SaathiAI learned / approvals waiting / closer to $7,938,838.98?), each sourced from a real subsystem. Publishes `executive.financial_briefing`.
- ⬜ Remaining to close M5: **M5 Integration Sprint** (Dev Rule #3 — verify every finance component emits standardized events, episodes flow to the shared Learning Runtime, KPIs registered, Executive consumes Mission Control not duplicate math, L4 governance + approval impossible to bypass, reconcile BUILD_STATUS/Brain.md/Wisdom.md) → tag `v0.4.0-finance`.
...
| **3 / B** | **Content Factory (autonomous production line)** | ✅ **Complete (2026-07-03)** — `saathi/content_factory.py` (4 tests) + `saathi/revenue.py` (5 tests) + `saathi/audience.py` (4 tests). **Content Factory** dispatches one content item to real platform posters (FB/IG/X/LinkedIn adapters, guarded) each gated by Discovery; publishes + analytics feed the Learning Runtime. **Revenue Engine** — unified `record_revenue()` across 12 sources, multi-currency→USD, **lights the CEO Dream Meter live**. **Audience Intelligence** — every video's watch-time/CTR/retention/subscribers/revenue becomes a structured Episode with a cluster-ready intent (e.g. `video_6-8min_whiteboard`), so the Promotion Engine can learn "6-8min whiteboard videos retain +23%". The full loop is closed: produce → gate → publish → analytics → learn → better videos. |
| 4 | HCG Live Signal × Research + Learning | ⬜ |
| **5 / Business OS** | **BusinessOS core + HCG Cafeteria + KPI Engine** | ✅ **Complete (2026-07-03)** — `saathi/business.py`, 7 tests. Universal `record_activity()` (any business → Episode + Revenue + Learning; travel/subscriptions/AI-Studio plug in the same way). **HCG Cafeteria** integrated: a day's production → Episode with a deterministic recommendation (Dal Bhat 180 produced / 171 sold / 9 waste → *"Reduce tomorrow by 6 portions"*, safety-buffered) + POS revenue → Dream Meter. **Enterprise KPI Engine** — define North Star + KPIs (higher/lower-is-better), record values, compare vs target; the backbone of Executive Intelligence. Remaining departments (Travel, Telegram subs, Personal Finance, Investment Intelligence, Opportunity Intelligence) plug into the same `record_activity()`. |
