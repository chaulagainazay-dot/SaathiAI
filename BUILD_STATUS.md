...
- ⬜ Dependency-ordered next slices (Ajay's graph): Opportunity Discovery (autonomous daily scan) → wire Due Diligence/Simulation/Risk/Recommendation to consume Research → Portfolio Intelligence (concentration/correlation/cash) → Execution Connectors (post-L4-approval only) → Investment Learning → Financial Mission Control (the *consumer*, built last). Tag `v0.4.0-finance` when complete.
